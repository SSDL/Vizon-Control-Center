		...
		.items-list-wrapper: table(cellpadding=0, cellspacing=0, class=sortable ? 'sortable' : false, data-list-path=list.path).table.items-list
			...
			tbody
				each item in items.results
					...
					else if list.key == 'Mission'
						for authuser in item.authorizedUsers
							if authuser._id.equals(user._id)
								+row(list, colums, item)
					else if list.key == 'User'
						if item._id.equals(user._id)
							+row(list, colums, item)
					else if list.key == 'TAP'
						- console.log(item);
						- var missions = item.get('missionId').toObject();
						each mission in missions
							- console.log("m", missions);
							each authuser in mission.authorizedUsers
								- console.log("au", authuser, user._id);
								if authuser.equals(user._id)
									- console.log("in here");
									+row(list, colums, item)
