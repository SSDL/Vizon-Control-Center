...
exports = module.exports = function(req, res) {
	...
	var renderView = function() {
		...
		if ( req.list.key == 'TAP' || req.list.key == 'CAP') 
			query.populate('missionId');
			...
			async.eachSeries(req.list.initialFields, compileFields , function() {
				if (!req.user.isSuper) {
					...
					items.results.forEach(function(item) {
						if ( req.list.key == 'Mission' ) {
							for ( var a in item.authorizedUsers.toObject() ) {
								var authuser = item.authorizedUsers[a];
								if (authuser._id.equals(req.user._id))
									items2.push(item)
							}
						} else if (req.list.key == 'User') {
							if (item._id.equals(user._id))
								items2.push(item);
						} else if (req.list.key == 'TAP' || req.list.key == 'CAP') {
							var missions = item.missionId;
							for (var m in missions.toObject()) {
								var mission = missions[m];
								for ( var a in mission.authorizedUsers.toObject() ) {
									var authuser = mission.authorizedUsers[a];
									if (authuser.equals(req.user._id)) {
										items2.push(item);
										return;
									}
								}
